"""HACCP evaluation orchestration for the paper-aligned pipeline."""
...
OUTPUT_DIR = Path("evaluation_results/haccp_pasteurization_10")

SAMPLE_BATCHES: int | None = 10
TRAIN_FRACTION = 0.60
...
PIPELINE_RULE_MAX_PREDICATES = 4
...
@dataclass(frozen=True)
class HaccpEvaluationResult:
    """Artifacts returned by the HACCP evaluation driver."""
...
def main() -> None:
    """Run the default HACCP evaluation."""
    print("Starting HACCP evaluation ...", flush=True)
    run_haccp_evaluation(report=True)


def run_haccp_evaluation(
    dataset_dir: Path = DATASET_DIR,
    output_dir: Path = OUTPUT_DIR,
    sample_batches: int | None = SAMPLE_BATCHES,
    train_fraction: float = TRAIN_FRACTION,
    report: bool = True,
    progress: bool | None = None,
) -> HaccpEvaluationResult:
    """Run HACCP discovery, validation, and post-hoc rule evaluation."""
    if progress is None:
        progress = report
    _progress(progress, f"Loading HACCP data from {dataset_dir} ...")
    sample = load_haccp_sample(dataset_dir, sample_batches)
    _progress(progress, f"Loaded {len(sample.runs)} complete batch run(s).")
    split = split_haccp_cases(sample, train_fraction)
    _progress(
        progress,
        (
            "Split batches into "
    ...
    _progress(
        progress,
        (
            "Running discovery pipeline with CVC5 SyGuS "
            f"(rule_max_predicates={PIPELINE_RULE_MAX_PREDICATES}) ..."
        ),
    )
    discovery = run_pipeline(PipelineConfig(
        ...
        rule_max_predicates=PIPELINE_RULE_MAX_PREDICATES,
    ...
    _progress(
        progress,
        f"Discovered {discovery.n_classes} SyGuS rule activit(y/ies).",
    )
    _progress(progress, "Validating held-out HACCP cases ...")
    validation = validate_sensor_log(
        test_data,
        discovery,
        case_boundaries=test_boundaries,
    )
    _progress(progress, "Computing rule metrics and writing artifacts ...")
    ...
    return HaccpEvaluationResult(
        summary=summary,
        comparison=comparison,
        train_best_rules=train_best_rules,
        test_best_rules=test_best_rules,
...
def write_haccp_outputs(
    ...
    """Write HACCP evaluation artifacts as CSV files."""
...
def _artifact_paths(output_dir: Path) -> dict[str, str | None]:
    output_dir.mkdir(parents=True, exist_ok=True)
    paths = {
...
def _summary_table(
    ...
    return pd.DataFrame([{
        ...
        "pipeline_rule_max_predicates": PIPELINE_RULE_MAX_PREDICATES,
